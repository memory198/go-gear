...
// 携带键值对字段（slog 语义）：text 追加 user_id=123，JSON 输出独立字段
logger.Info(ctx, "user created", "user_id", 123, "source", "api")

// 无 ctx 打印（log 风格，INFO 级，不带 trace）：用于启动、后台任务等非请求场景
logger.Print("server starting", "port", 8080)
...
l, _ := logger.New(logger.Config{
    ...
    MaxAge:  30,                    // 日志保留天数
    Caller:  true,                  // 记录一行调用位置（非调用栈）
    ParentSpanIDs: false,           // 是否输出中间 span 链 parent_span_ids（默认 false，开启需聚合）
})
logger.SetDefault(l)
defer logger.Close()
```
...
```
// text 格式（默认）
2026-09-21T10:30:00.123456+08:00 [INFO] handler/user.go:42 creating user
2026-09-21T10:30:00.123456+08:00 [INFO] [abc123] handler/user.go:42 creating user  // 有 trace 时

// json 格式（字段对齐 OTel）
{"timestamp":"2026-09-21T10:30:00.123456+08:00","severity_number":9,"severity_text":"INFO",
 "body":"creating user","code.filepath":"handler/user.go","code.lineno":42,
 "trace_id":"abc123","span_id":"s3",
 "resource":{"service.name":"user-api"},"attributes":{"user_id":123,"source":"api"}}
```

> 日志也可选择通过 OTLP 直接上报到 OTel Collector（与 trace 同一后端互跳），见 [OTel接入指南](OTel接入指南.md) 的“日志 OTLP 导出”章节。
...
go-gear 不内置路由，**推荐搭配 [chi](https://github.com/go-chi/chi)（v5）作为路由层**。完整接入指南（安装、参数绑定、中间件顺序、完整示例）见 **[chi路由接入指南](chi路由接入.md)**。

核心结论：零适配代码——`framework.Handle` 返回标准 `http.HandlerFunc`，go-gear 中间件签名 `func(http.Handler) http.Handler` 与 `chi.Use` 直接兼容：
...
**路径参数绑定**：让 `Req` 实现 `framework.Binder`，在 `Bind` 里用 `chi.URLParam(r, "id")` 取参数（详见 [chi路由接入指南](chi路由接入.md#路径参数chiurlparam--binder)）。
...
log:
  ...
  max_age: 30
  caller: true
  # resource 元信息（OTel Resource 语义约定）
  service: user-api
  version: "1.2.0"
  env: prod
  host: node-1
...
func createUser(ctx context.Context, req *CreateUserReq) (*User, error) {
    ...
    // json 格式输出示例（字段对齐 OTel）：
    // {"timestamp":"2026-09-21T10:30:00.123456+08:00","severity_text":"INFO",
    //  "body":"creating user","trace_id":"a1b2c3...","span_id":"d4e5f6..."}
